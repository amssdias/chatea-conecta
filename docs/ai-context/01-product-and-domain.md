...
| Current subscription | Stripe subscription ID currently stored on the one-to-one local subscription | `UserSubscription.stripe_subscription_id` | Checkout completion may replace it; later events for other IDs are ignored. |
