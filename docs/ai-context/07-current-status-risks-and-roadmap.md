...
| Medium | **Confirmed: message abuse limits are connection-local** | `handle_send_message` validates type/blank/1,000-character maximum and limits each socket to eight attempts per five seconds, but reconnects and parallel sockets start fresh quotas | One socket cannot flood the room continuously, but one identity can multiply its permitted rate across connections | If abuse warrants it, add an atomic Redis quota keyed by trusted identity/IP plus communicator tests for multi-socket behavior |
